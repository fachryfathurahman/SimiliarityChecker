...
def count_line_frequency(line_list_1, line_list_2):
    line_list_1 = set(line_list_1)
    line_list_2 = set(line_list_2)
    same_line = line_list_1.intersection(line_list_2)
...
def documentSimiliarity(filename_1, filename_2):
    ...
    distance = abs(((np.rad2deg(distance) / 90) * 100)-100)
    average_percentage = (sorted_line_list+(abs(((np.rad2deg(distance) / 90) * 100) - 100)))/2
    list = [distance, sorted_line_list]
    return list
